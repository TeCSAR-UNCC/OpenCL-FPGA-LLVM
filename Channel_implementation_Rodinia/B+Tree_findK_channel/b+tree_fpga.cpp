...
void kernel_gpu_opencl_wrapper(cl_context context, record *records, long records_mem, knode *knodes, long knodes_elem, long knodes_mem, int order, long maxheight, int count,long *currKnode,long *offset,int *keys,record *ans){
	
    // 1. set up kernel
		cl_kernel btree_findk_kernel_read, btree_findk_kernel_compute, btree_findk_kernel_store;
        cl_int status;
        float writeTime=0, kernel_Time=0, readTime=0;
        cl_program cl_btreefindk_program;
        cl_btreefindk_program = cl_compileProgram((char *)"./binary/b+tree_findK_default.aocx",NULL);
        btree_findk_kernel_read = clCreateKernel(cl_btreefindk_program, "findK_Read", &status);
        status = cl_errChk(status, (char *)"Error Creating btree findK Read kernel",true);
        if(status)exit(1);
        btree_findk_kernel_compute = clCreateKernel(cl_btreefindk_program, "findK_compute", &status);
        status = cl_errChk(status, (char *)"Error Creating btree findK compute kernel",true);
        if(status)exit(1);
        btree_findk_kernel_store = clCreateKernel(cl_btreefindk_program, "findK_store", &status);
        status = cl_errChk(status, (char *)"Error Creating btree findK store kernel",true);
        if(status)exit(1);
...
//////////////////////////////////////////////////////////////////////// 
    ...
    cl_mem recordsD,knodesD,currKnodeD,offsetD,keysD,ansD;
	
    cl_int error=0;

    recordsD = clCreateBuffer(context, CL_MEM_READ_WRITE,records_mem,NULL,&error);
    ...
    keysD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(long),NULL,&error);
    ansD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(record),NULL,&error); 
	cl_errChk(error,"ERROR Creating Device memory",true);
    ...
    error = clEnqueueWriteBuffer(command_queue,keysD,1,0,count*sizeof(int),keys,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,ansD,1,0,count*sizeof(record),ans,0,NULL,&writeEvent);
    cl_errChk(error,"ERROR in Copy data from host to device",true);
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////

	long long start_time = get_time();
    cl_int argchk;
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_findk_kernel_read,0,sizeof(cl_mem),(void *) &currKnodeD);
		argchk  = clSetKernelArg(btree_findk_kernel_read,1,sizeof(cl_mem),(void *) &offsetD);
		argchk  = clSetKernelArg(btree_findk_kernel_read,2,sizeof(cl_mem),(void *) &keysD);
		cl_errChk(argchk,"ERROR in Setting btree findK Read kernel args",true);
	
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_findk_kernel_compute,0,sizeof(long),(void *) &maxheight);
		argchk  = clSetKernelArg(btree_findk_kernel_compute,1,sizeof(cl_mem),(void *) &knodesD);
		argchk  = clSetKernelArg(btree_findk_kernel_compute,2,sizeof(long),(void *) &knodes_elem);
		argchk  = clSetKernelArg(btree_findk_kernel_compute,3,sizeof(cl_mem),(void *) &recordsD);
		cl_errChk(argchk,"ERROR in Setting btree findK compute kernel args",true);	

	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_findk_kernel_store,0,sizeof(cl_mem),(void *) &offsetD);
		argchk  = clSetKernelArg(btree_findk_kernel_store,1,sizeof(cl_mem),(void *) &ansD);
		cl_errChk(argchk,"ERROR in Setting btree findK kernel args",true);
			

	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue,btree_findk_kernel_read,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing b+tree findK Read Kernel",true);
		//printf("b_tree findK kernel complete\n");
	
	
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue1,btree_findk_kernel_compute,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing b+tree findK compute Kernel",true);
		//printf("b_tree findK kernel complete\n");
	
	
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue2,btree_findk_kernel_store,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing b+tree findK store Kernel",true);
		//printf("b_tree findK store kernel complete\n");
...
int main(int argc, char * argv[]){
  ...
  // terminate
	...
	// OpenCL kernel
	kernel_gpu_opencl_wrapper(context, records,records_mem,knodes,knodes_elem,knodes_mem,order,maxheight,count,currKnode,offset,keys,ans);
