...
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////
    ...
    do{
		...
		// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue2,BFS_kernel_2, 1, 0,globalWorkSize,NULL,0, NULL, &kernelEvent3);
		//kernel_2+=eventTime(kernelEvent3,command_queue2);
		clReleaseEvent(kernelEvent3);
		//~ printf("kernel 2 Executes\n");
		cl_errChk(error,"ERROR in Executing Kernel BFS_2",true);
		
		status = clFinish( command_queue2);
		
		error = clEnqueueReadBuffer(command_queue2, d_over, 1, 0, sizeof(char), &h_over, 0,NULL,&readEvent);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////END OF OPERATION
    ...
    if (1) {
        ...
        printf("# Records\tWrite(s)\tKernel_Read(s)\tKernel_Compute(s)\tKernel_Wb(s)\tRead(s)\n");
        printf("%d        \t",no_of_nodes);
        // Write Buffer
        printf("%f \t",writeTime);
       	// Kernel_read
        error = clGetEventProfilingInfo(kernelEvent,CL_PROFILING_COMMAND_START,sizeof(cl_ulong),&eventStart,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel Start)",true); 
        error = clGetEventProfilingInfo(kernelEvent,CL_PROFILING_COMMAND_END,sizeof(cl_ulong),&eventEnd,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel End)",true);
        printf("%f\t",kernel_read);
        // Kernel_Compute
        error = clGetEventProfilingInfo(kernelEvent1,CL_PROFILING_COMMAND_START,sizeof(cl_ulong),&eventStart,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel Start)",true); 
        error = clGetEventProfilingInfo(kernelEvent1,CL_PROFILING_COMMAND_END,sizeof(cl_ulong),&eventEnd,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel End)",true);
        printf("%f\t\t",kernel_compute);
        // Kernel_wb
        error = clGetEventProfilingInfo(kernelEvent2,CL_PROFILING_COMMAND_START,sizeof(cl_ulong),&eventStart,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel Start)",true); 
        error = clGetEventProfilingInfo(kernelEvent2,CL_PROFILING_COMMAND_END,sizeof(cl_ulong),&eventEnd,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Kernel End)",true);
        printf("%f \t",kernel_store);
        // Read Buffer
        error = clGetEventProfilingInfo(readEvent,CL_PROFILING_COMMAND_START,sizeof(cl_ulong),&eventStart,NULL);
        cl_errChk(error,"ERROR in Event Profiling (Read Start)",true); 
        error = clGetEventProfilingInfo(readEvent,CL_PROFILING_COMMAND_END,sizeof(cl_ulong),&eventEnd,NULL);
