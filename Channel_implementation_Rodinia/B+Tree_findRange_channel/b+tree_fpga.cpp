...
void kernel_gpu_opencl_wrapper_2(cl_context context, knode *knodes,long knodes_elem,long knodes_mem,int order,long maxheight,int count,long *currKnode,long *offset,long *lastKnode,long *offset_2,int *start,int *end,int *recstart,int *reclength){
	
    // 1. set up kernel
		cl_kernel btree_kernel_read,btree_kernel_compute,btree_kernel_store ;
        cl_int status;
        float writeTime=0, kernel_Time=0, readTime=0;
        cl_program cl_btree_program;
        cl_btree_program = cl_compileProgram((char *)"./binary/b+tree_findRangeK_default.aocx",NULL);
	       
        btree_kernel_read = clCreateKernel(cl_btree_program, "findRangeK_Read", &status);
        status = cl_errChk(status, (char *)"Error Creating btree kernel",true);
        if(status)exit(1);
        
        btree_kernel_compute = clCreateKernel(cl_btree_program, "findRangeK_compute", &status);
        status = cl_errChk(status, (char *)"Error Creating btree kernel",true);
        if(status)exit(1);
       
        btree_kernel_store = clCreateKernel(cl_btree_program, "findRangeK_store", &status);
        status = cl_errChk(status, (char *)"Error Creating btree kernel",true);
        if(status)exit(1);
...
//////////////////////////////////////////////////////////////////////// 
    ...
    cl_mem knodesD,currKnodeD,offsetD,lastKnodeD,offset_2D,startD,endD,ansDStart,ansDLength;
	
    cl_int error=0;

    knodesD = clCreateBuffer(context,CL_MEM_READ_WRITE,knodes_mem,NULL,&error );
    currKnodeD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(long),NULL,&error );
    offsetD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(long),NULL,&error );
    lastKnodeD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(long),NULL,&error );
    offset_2D = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(long),NULL,&error );
    startD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(int),NULL,&error );
    endD = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(int),NULL,&error );
    ansDStart = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(int),NULL,&error );
    ansDLength = clCreateBuffer(context,CL_MEM_READ_WRITE,count*sizeof(int),NULL,&error );
	...
	cl_event writeEvent,kernelEvent,kernelEvent2,kernelEvent3,readEvent;
    ...
    error = clEnqueueWriteBuffer(command_queue,currKnodeD,1,0,count*sizeof(long),currKnode,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,offsetD,1,0,count*sizeof(long),offset,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,lastKnodeD,1,0,count*sizeof(long),lastKnode,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,offset_2D,1,0,count*sizeof(long),offset_2,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,startD,1,0,count*sizeof(int),start,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,endD,1,0,count*sizeof(int),end,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,ansDStart,1,0,count*sizeof(int),recstart,0,NULL,&writeEvent);
    error = clEnqueueWriteBuffer(command_queue,ansDLength,1,0,count*sizeof(int),reclength,0,NULL,&writeEvent);		
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////

	long long start_time = get_time();
    cl_int argchk;
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_kernel_read,0,sizeof(cl_mem),(void *) &currKnodeD);
		argchk  = clSetKernelArg(btree_kernel_read,1,sizeof(cl_mem),(void *) &lastKnodeD);
		argchk  = clSetKernelArg(btree_kernel_read,2,sizeof(cl_mem),(void *) &startD);
		argchk  = clSetKernelArg(btree_kernel_read,3,sizeof(cl_mem),(void *) &endD);
		argchk  = clSetKernelArg(btree_kernel_read,4,sizeof(cl_mem),(void *) &ansDStart);
		cl_errChk(argchk,"ERROR in Setting btree Read kernel args",true);
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue,btree_kernel_read,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing btree Read Kernel",true);
		//printf("b_tree Read kernel complete\n");
	
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_kernel_compute,0,sizeof(long),(void *) &maxheight);
		argchk  = clSetKernelArg(btree_kernel_compute,1,sizeof(cl_mem),(void *) &knodesD);
		argchk  = clSetKernelArg(btree_kernel_compute,2,sizeof(long),(void *) &knodes_elem);
		cl_errChk(argchk,"ERROR in Setting btree Compute kernel args",true);
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue1,btree_kernel_compute,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent2);
		cl_errChk(error,"ERROR in Executing btree compute Kernel",true);
		//printf("b_tree Compute kernel complete\n");
	
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_kernel_store,0,sizeof(cl_mem),(void *) &offsetD);
		argchk  = clSetKernelArg(btree_kernel_store,1,sizeof(cl_mem),(void *) &offset_2D);
		argchk  = clSetKernelArg(btree_kernel_store,2,sizeof(cl_mem),(void *) &ansDStart);
		argchk  = clSetKernelArg(btree_kernel_store,3,sizeof(cl_mem),(void *) &ansDLength);
		cl_errChk(argchk,"ERROR in Setting btree Store kernel args",true);
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue2,btree_kernel_store,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent3);
		cl_errChk(error,"ERROR in Executing btree Store Kernel",true);
		//printf("b_tree Store kernel complete\n");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////END OF OPERATION
    ...
    error = clEnqueueReadBuffer(command_queue,ansDStart,CL_TRUE,0,count*sizeof(int),recstart,0,NULL,NULL);
	error = clEnqueueReadBuffer(command_queue,ansDLength,CL_TRUE,0,count*sizeof(int),reclength,0,NULL,NULL);
...
int main(int argc, char * argv[]){
  ...
  // terminate
     ...
     if (pFile==NULL) 
       ...
       printf("file to open file output\n");
	...
	sscanf(commandPointer, "%c", &instruction);
	commandPointer++;
	printf("%c\n",instruction);
	int count;
	...
	kernel_gpu_opencl_wrapper_2(context, knodes, knodes_elem,knodes_mem,order,maxheight,count,currKnode,offset,lastKnode,offset_2,start,end,recstart,reclength);
