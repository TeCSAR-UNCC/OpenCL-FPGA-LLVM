...
int main(int argc, char** argv) {
  ...
  printf("WG size of kernel = %d X %d\n", BLOCK_SIZE, BLOCK_SIZE);
    ...
    readinput(FilesavingPower, grid_rows, grid_cols, pfile);
        ...
        // create kernel
        char* hotspot_kernel_file = "./binary/hotspot_default_128.aocx";
        char* hotspot_kernel_name = "hotspot";
        cl_program program = NULL;
        printf("begin to make kernel hotspot\n");
        kernel = make_kernel(context, program, device, hotspot_kernel_file, hotspot_kernel_name); 
        printf("kernel hotspot is created\n");
	...
	int ret = compute_tran_temp(MatrixPower, MatrixTemp, grid_cols, grid_rows, total_iterations, pyramid_height, blockCols, blockRows, borderCols, borderRows, FilesavingTemp, FilesavingPower);
