...
void kernel_gpu_opencl_wrapper_2(cl_context context, knode *knodes,long knodes_elem,long knodes_mem,int order,long maxheight,int count,long *currKnode,long *offset,long *lastKnode,long *offset_2,int *start,int *end,int *recstart,int *reclength){
    ...
    // 1. set up kernel
        ...
        btree_kernel = clCreateKernel(cl_btree_program, "findRangeK", &status);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////
	...
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_kernel,0,sizeof(long),(void *) &maxheight);
		argchk  = clSetKernelArg(btree_kernel,1,sizeof(cl_mem),(void *) &knodesD);
		argchk  = clSetKernelArg(btree_kernel,2,sizeof(long),(void *) &knodes_elem);
		argchk  = clSetKernelArg(btree_kernel,3,sizeof(cl_mem),(void *) &currKnodeD);
		argchk  = clSetKernelArg(btree_kernel,4,sizeof(cl_mem),(void *) &offsetD);
		argchk  = clSetKernelArg(btree_kernel,5,sizeof(cl_mem),(void *) &lastKnodeD);
		argchk  = clSetKernelArg(btree_kernel,6,sizeof(cl_mem),(void *) &offset_2D);
		argchk  = clSetKernelArg(btree_kernel,7,sizeof(cl_mem),(void *) &startD);
		argchk  = clSetKernelArg(btree_kernel,8,sizeof(cl_mem),(void *) &endD);
		argchk  = clSetKernelArg(btree_kernel,9,sizeof(cl_mem),(void *) &ansDStart);
		argchk  = clSetKernelArg(btree_kernel,10,sizeof(cl_mem),(void *) &ansDLength);
		cl_errChk(argchk,"ERROR in Setting btree kernel args",true);
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue,btree_kernel,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing Hotspot Kernel",true);
		printf("b_tree kernel complete\n");
