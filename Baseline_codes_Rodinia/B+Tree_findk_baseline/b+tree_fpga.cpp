...
void kernel_gpu_opencl_wrapper(cl_context context, record *records, long records_mem, knode *knodes, long knodes_elem, long knodes_mem, int order, long maxheight, int count,long *currKnode,long *offset,int *keys,record *ans){
    ...
    // 1. set up kernel
        ...
        btree_findk_kernel = clCreateKernel(cl_btreefindk_program, "findK", &status);
        status = cl_errChk(status, (char *)"Error Creating btree findK kernel",true);
        if(status)exit(1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////
	...
	// 3. send arguments to device
		argchk  = clSetKernelArg(btree_findk_kernel,0,sizeof(long),(void *) &maxheight);
		argchk  = clSetKernelArg(btree_findk_kernel,1,sizeof(cl_mem),(void *) &knodesD);
		argchk  = clSetKernelArg(btree_findk_kernel,2,sizeof(long),(void *) &knodes_elem);
		argchk  = clSetKernelArg(btree_findk_kernel,3,sizeof(cl_mem),(void *) &recordsD);
		argchk  = clSetKernelArg(btree_findk_kernel,4,sizeof(cl_mem),(void *) &currKnodeD);
		argchk  = clSetKernelArg(btree_findk_kernel,5,sizeof(cl_mem),(void *) &offsetD);
		argchk  = clSetKernelArg(btree_findk_kernel,6,sizeof(cl_mem),(void *) &keysD);
		argchk  = clSetKernelArg(btree_findk_kernel,7,sizeof(cl_mem),(void *) &ansD);
		cl_errChk(argchk,"ERROR in Setting btree findK kernel args",true);
	// 4. enqueue kernel
		error = clEnqueueNDRangeKernel(command_queue,btree_findk_kernel,1,NULL,global_work_size,local_work_size,0,NULL,&kernelEvent);
		cl_errChk(error,"ERROR in Executing b+tree findK Kernel",true);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////END OF OPERATION
    ...
    error = clEnqueueReadBuffer(command_queue,ansD,CL_TRUE,0,count*sizeof(record),ans,0,NULL,&readEvent);
